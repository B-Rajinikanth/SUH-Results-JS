...
const loadExcel = () => {
	fetch("A24_Sem_1_Supply_Results.xlsx")
...
const getResult = () => {
	...
	if (!studentResult) {
		...
		document.getElementById("sorRes").innerHTML = `Semester: <span class="font-semibold">I</span>`;
		...
		studentResult.courses.forEach(course => {
			...
			if(course.result) {
				course.result = course.result.trim();
				// console.log(subjectCode + " - " + course.result + " - " + course.result.length);
			}
			if (course.result !== undefined && course.result !== "") {
